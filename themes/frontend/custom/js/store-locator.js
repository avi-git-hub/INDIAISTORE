...
function get_nearby_partner_stores_data(lat, lng, state_id, city_id, distance) {
  var ajax_call = $.ajax({
    data: { lat, lng, state_id, city_id, distance },
    dataType: "json",
    type: "post",
  ...
  ajax_call.done(function (response, status) {
    ...
    if (response.status == true) {
      ...
      if (response.data.offline_stores) {
        $("#offline_stores_section")
          .hide()
          .html(response.data.offline_stores)
          .fadeIn("slow");
      ...
      if (response.data.online_stores) {
        $("#online_stores_section")
          .hide()
          .html(response.data.online_stores)
          .fadeIn("slow");
...
$("#nearby").click(function (e) {
  ...
  if (location_status == "denied") {
    alert(
      "Please turn on your location services and allow us to find your nearest store."
    );
  } else {
    getPosition();
  }
...
$(function () {
  ...
  $("#select-city-dd")
    .select2({ placeholder: "Select City" })
    .on("select2:selecting", function (e) {
