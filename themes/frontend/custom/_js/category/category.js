...
function get_prod_by_price_range() {
  ...
  ajax_call.done(function ({ status: s, data }, status) {
    ...
    if (s == true) {
      var html = "";
      for (let p of data) {
        var bg_color = p.prod_card_bg_color
          ? `background-color:${p.prod_card_bg_color}`
          : "";
      ...
      switch (true) {
        case data.length > 1:
          ...
          if (["iphone", "ipad"].includes(cat_slug)) {
            $(".recommendation .btn")
              .html("Compare")
              .attr("href", `${base_url}compare/${cat_slug}?${url}`)
              .show();
          } else {
            $(".recommendation .btn")
              .html("Compare")
              .attr("href", `${base_url}compare/${cat_slug}`)
              .show();
          }
        ...
        default:
          $(".recommendation .versus-products").html(
            "<p class='mt-3 w-100'>No Results Found! Please use different values for better result.</p>"
          );
          $(".recommendation .btn").hide();
          break;
      }
...
function filter_products() {
  var sort_product_by = $(".sort-options")
    .find("a")
    .filter(".active")
    .data("value");
      ...
      if (response.status == true) {
        ...
        for (i; i < filtered_prod_count; i++) {
          ...
          markup_1 +=
            ...
            "/" +
            filtered_products[i].prod_img_md +
            '">';
          markup_1 += "   </div>";

          markup_1 +=
            '   <div class="name">' + filtered_products[i].prod_name + "</div>";

          markup_1 +=
            '   <div class="price">₹<span>' +
            filtered_products[i].prod_mrp +
            "</span></div>";
          markup_1 += " </div>";
          markup_1 += "</div>";
          ...
          markup_2 +=
            ' <div class="product" data-prod-name="' +
          ...
          if (
            ...
            filtered_products[i].prod_effective_price > 0
          ) {
            markup_2 +=
              '<div class="price">₹ <span>' +
              filtered_products[i].prod_effective_price +
          ...
          if (filtered_prod_images_count > 0) {
            ...
            if (
              filtered_products[i].prod_vid_id != null &&
              filtered_products[i].prod_vid_thumbnail != null
            ) {
              markup_2 += '           <div class="shot-slider-item">';
              markup_2 +=
...
$(document).on("click", ".cat-compare-btn", function (e) {
  ...
  if (!$(this).hasClass("added-to-compare")) {
    ...
    compare_float_btn.find(".total-product span").text(
      compare_prods_array.length.toLocaleString("en-IN", {
    ...
    var index = compare_prods_array.indexOf(
      $.trim($(this).attr("data-prod-slug"))
    );
    if (index !== -1) {
