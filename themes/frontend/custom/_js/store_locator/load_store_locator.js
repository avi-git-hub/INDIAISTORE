          ...
          for (i; i < arr1_length; i++) {
            ...
            google.maps.event.addListener(marker, 'click', function (marker, i) {
              return function () {
                ...
                $(".store[data-store-id='" + (i + 1) + "']").find('.cta_get_directions').trigger('click');
              };
            }(marker, i));
          }
...
/* function showPosition(position) {
  ...
  if (Object.keys(urlParams).length > 0) {
    ...
    window.localStorage.setItem("user_location_access", "allow");
    window.localStorage.setItem("user_location_distance", "5");

    // $(".nearby_link").removeClass("invisible");
    // $(".nearby_link_main").removeClass("hidden");
    ...
    if (window.localStorage.getItem("user_location_distance")) {
      distance = window.localStorage.getItem("user_location_distance");
    } else {
      distance = 5;
    }

    get_nearby_partner_stores_data(lat, lng, distance);
  }
}
...
function accessDenied() {
  ...
  if (Object.keys(urlParams).length > 0) {
    ...
    // $(".nearby_link").addClass("invisible");
    // $(".nearby_link_main").addClass("hidden");
...
function draw_coords(chosen_state, chosen_city) {
  $.ajax({
    ...
    success: function success(response, status) {
      ...
      //$("#aar_sec").css("height", (is_mobile) ? "" : "575px");

      //$("#multibrand_sec").css("height", (is_mobile) ? "" : "575px");
...
function loadMapMarker(store) {
  ...
  store_wrapper_html += "<div class='store_details store_tile' data-store-type='" + $(store).attr('data-store-type') + "' data-store-id='" + $(store).attr('data-store-id') + "' data-storename='" + $(store).attr('data-storename') + "' data-address='" + $(store).attr('data-address') + "' data-pincode='" + $(store).attr('data-pincode') + "' data-phone='" + $(store).attr('data-phone') + "'>" + $(store).find('.store_details').html() + '</div>';
...
$('#frmstate').on('change', function () {
  ...
  $('#frmstate_2').find("option[value='" + chosen_state + "']").attr('selected', 'selected');
  ...
  if (chosen_state !== '') {
    $.ajax({
      ...
      success: function success(cities) {
        ...
        if (no_of_cities > 0) {
          $('#frmcity, #frmcity_2').empty().append($('<option>', {
            value: '',
            text: 'Select City'
          }));

          for (i; i < no_of_cities; i++) {
            $('#frmcity, #frmcity_2').append($('<option>', {
              value: cities[i].id,
              text: cities[i].text
            }));
          }
...
$('#frmcity').on('change', function () {
  $('#frmcity_2').find("option[value='" + $(this).val() + "']").attr('selected', 'selected');
});
...
$('#frmstate_2').on('change', function () {
  ...
  $('#frmstate').find("option[value='" + chosen_state + "']").attr('selected', 'selected');
...
$('#frmcity_2').on('change', function () {
  $('#frmcity').find("option[value='" + $(this).val() + "']").attr('selected', 'selected');
});
...
$(document).on('click', '#btn_send_sms', function (e) {
  ...
  if (field_mob_num !== '' && field_store_name !== '' && field_store_address !== '' && field_store_pincode !== '' && field_store_phone !== '' // && (field_store_is_open !== "")
  ) {
        ...
        if (field_mob_num.length === 10) {
          ...
          $.ajax({
            ...
            success: function success(response) {
              $('.field_msg').addClass(response.status === true ? 'alert-success' : 'alert-danger').html(response.message).removeClass('hidden');
...
$(document).ready(function () {
  ...
  $('#form_state_city_2').validate({
    ...
    submitHandler: function submitHandler(form) {
      ...
      if (chosen_state !== '' && chosen_city !== '') {
        // executing this code only on live
        /* if ($("#env").attr("data-value") == "production") {
          ga("send", {
            hitType: "event",
            eventCategory: "Store Locator",
            eventAction: "Geo Selection",
            eventLabel: (chosen_state + "," + chosen_city)
          });
           console.log("Store Locator event: " + (chosen_state + "," + chosen_city));
        } */
