...
function get_product_pricing() {
  ...
  console.log(selected_product, selected_storage); 
  ...
  if(selected_product == 'mac-studio-m2'){
    //alert($(this).data("value"));

    var processor_new = $(".selection.processor .attribute.selected").data('value');

    console.log("Processor"+processor_new);

    if (processor_new == "m2-ultra") {
  
      $(".price-calculator .selection.rect .attribute:eq(1)").addClass("selected").show();
      $(".price-calculator .selection.rect .attribute:eq(0)").hide();
    } else if(processor_new == "m2-max"){
      
      $(".price-calculator .selection.rect .attribute:eq(0)").addClass("selected").show();
      $(".price-calculator .selection.rect .attribute:eq(1)").hide();
    }
  }
  ...
  ajax_call.done(function (response, status) {
    if (response.status == true) {
      ...
      $(".sliderButton").click(function () {
        ...
        $(".product-info .tns-item.active").find(".product-shot").addClass("active");
...
$(document).on("click", ".product-info .price-calculator .selection.processor .attribute", function (e) {
  ...
  var storage = $(".selection.rect .attribute.selected").data('value');
  
  var prodslug = $("#prodslubyproduct").text();

  console.log(storage, prodslug);
  if(prodslug == 'mac-mini-m2'){
    if ($(this).data("value") == "m2-pro") {
      $(".price-calculator .selection.rect .attribute:eq(0)").addClass("selected");
      $(".price-calculator .selection.rect .attribute:eq(1)").removeClass("selected").hide();
    } else if($(this).data("value") == "m2"){
      $(".price-calculator .selection.rect .attribute:eq(0)").addClass("selected");
      $(".price-calculator .selection.rect .attribute:eq(1)").show();
    }
  }  else if(prodslug == 'mac-studio-m2'){
    //alert($(this).data("value"));
    if ($(this).data("value") == "m2-ultra") {
      console.log(1+"mac-studio-m2"+11111111111111);
      $(".price-calculator .selection.rect .attribute:eq(1)").addClass("selected").show();
      $(".price-calculator .selection.rect .attribute:eq(0)").removeClass("selected").hide();
    } else if($(this).data("value") == "m2-max"){
      console.log(2+"mac-studio-m2"+2222222222);
      $(".price-calculator .selection.rect .attribute:eq(0)").addClass("selected").show();
      $(".price-calculator .selection.rect .attribute:eq(1)").hide();
    }
  }  else {
  if ($(this).data("value") == "m1-max" || $(this).data("value") == "i7" || $(this).data("value") == "m2-max" || $(this).data("value") == "m2") {
...
$("#field_switch_from").on("change", function (e) {
  ...
  if (switch_from_device) {
    ...
    var ajax_call = $.ajax({
      cache: false,
      data: {
        switch_from_device: switch_from_device
      },
      dataType: "json",
      type: "POST",
    ...
    ajax_call.done(function (response, status) {
      if (response.success == true) {
        ...
        if (switch_cal.length) {
          switch_cal.forEach(function (val, i) {
...
$(function () {
  ...
  $(document).on("click", ".copy-link-share-icon", function (e) {
    e.preventDefault();
    var elementText = $(this).attr("href");
