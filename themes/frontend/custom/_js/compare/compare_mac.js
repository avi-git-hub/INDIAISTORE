...
var reorderDOM = function reorderDOM(column, e) {
  var parents = document.querySelectorAll('[data-column-group]');
  var activeIndex = e.target.value;
  var currentColumn = column + 1; // skip over rowheaders

  var swapNodes = function swapNodes(target, ref) {
  ...
  parents.forEach(function (parent) {
    var targetChild = parent.querySelector(".product-" + activeIndex);
    var refChild = parent.children[currentColumn];

    swapNodes(targetChild, refChild);
  });
};

var setChangeEvent = function setChangeEvent() {
  var selects = document.querySelectorAll('.selector-dropdown');
  selects.forEach(function (select, i) {
    select.addEventListener('change', function (e) {
      return reorderDOM(i, e);
    });
  });
};
