    ...
    ajax_call.done(function (response, status) {
      if (response.status == true) {
        ...
        for (const [feature, feature_values] of Object.entries(applicable_features_data)) {
          ...
          if (typeof $("#applicable_features").data(feature) != "undefined") {
            selected_feature_values = $("#applicable_features").data(feature).toString();

            selected_feature_values = selected_feature_values.split(",");
          }
          ...
          for (i; i < feat_values_count; i++) {
            ...
            feature_data_markup += `<option value="${feat_value_id}"`;

            if (selected_feature_values.length > 0) {
              if ($.inArray(feat_value_id, selected_feature_values) != -1) {
                feature_data_markup += ` selected`;
              }
            }

            feature_data_markup += `>${feat_value_title}</option>`;
          }
        ...
        $("#applicable_features").empty().html(feature_data_markup);
...
function get_products_to_compare_with(cat_id, prod_id) {
  if ($("#product_category").val() == "") {
      ...
      if (response.status == true) {
        if (typeof response.info.products_to_compare_with != "undefined") {
          ...
          for (i; i < prods_count; i++) {
            option = $("<option>").val(prods_to_compare_with[i].prod_id).text(prods_to_compare_with[i].prod_name);

            $product_compare_with.append(option);
          }

          $("#product_compare_with").val($("#product_compare_with_prod_id").val()).trigger("change");
...
$("#product_category").on("change", function () {
  ...
  get_products_to_compare_with(cat_id, prod_id);
});
// --------------------------------------------------
// event handlers
// --------------------------------------------------
