$(document).ready(function () {
  ...
  $(document).on("click", ".add-row", function (e) {
    ...
    var deleteButton = `<a href="javascript:void(0)" role="button" class="text-danger delete-row">
    ...
    if (!partner_product_val) {
      ...
      if ($("table tbody tr").length < 1) {
        ...
        $("table tbody tr").each(function () {
          var id = Number($(this).find("td").html());
          if (id == partner_product_val) count = false;
        });
        if (count) {
          $("table tbody").append(markup);
          $("#partner_product,#partner_product_link").val(null).trigger("change");
  ...
  $(document).on("click", "#partner_submit", function (e) {
    ...
    $.ajax({
      ...
      processData: false,
      type: "POST",
      url: "/admin/partner/add-new",
      success: function (data) {
        if (data.success) {
          alert("Partner has been saved successfully!");
  ...
  $("#partner_is_pan_india").on("switchChange.bootstrapSwitch", function () {
    ...
    $("#partner_states,#partner_cities").val(null).trigger("change");
  });
});
